...
int OnReceivingForward( struct HetaoServer *p_server , struct HttpSession *p_http_session )
{
	...
	/* 收一把HTTP响应 */
	nret = ReceiveHttpResponseNonblock( p_http_session->netaddr.sock , NULL , p_http_session->http ) ;
	...
	else
	{
		...
		/* 复制HTTP响应 */
		response_base = GetHttpBufferBase( GetHttpRequestBuffer(p_http_session->forward_http) , & response_len ) ;
